...
public class BondarevTests {
    ...
    public void constructorWithValue_ShouldSetValueInValidRange() {
        executeStep("Проверка установки значения 3", () -> {
            assertEquals(3, new Mark(3).get(), DELTA);
        });

        executeStep("Проверка установки значения 7 (должно стать 5)", () -> {
            assertEquals(5, new Mark(7).get(), DELTA);
        });

        executeStep("Проверка установки отрицательного значения (должно стать 0)", () -> {
    ...
    public void plus_ShouldIncreaseWithinBounds() {
        ...
        executeStep("Увеличение оценки с 2 до 3", () -> {
            mark.plus();
            assertEquals(3, mark.get(), DELTA);
        });

        Mark maxMark = new Mark(5);
        executeStep("Попытка увеличения максимальной оценки 5 (должно остаться 5)", () -> {
            maxMark.plus();
    ...
    public void minus_ShouldDecreaseWithinBounds() {
        Mark mark = new Mark(2);
        executeStep("Уменьшение оценки с 2 до 1", () -> {
            mark.minus();
            assertEquals(1, mark.get(), DELTA);
        });

        Mark minMark = new Mark(0);
        executeStep("Попытка уменьшения минимальной оценки 0 (должно остаться 0)", () -> {
            minMark.minus();
    ...
    public void plusWithValue_ShouldHandleValidAndInvalidCases() {
        Mark mark = new Mark(2);
        executeStep("Увеличение оценки 2 на 2 = 4", () -> {
            mark.plus(2);
            assertEquals(4, mark.get(), DELTA);
        });

        executeStep("Увеличение оценки 4 на 10 (должно стать 5)", () -> {
            mark.plus(10);
            assertEquals(5, mark.get(), DELTA);
        });

        Mark anotherMark = new Mark(2);
        executeStep("Увеличение оценки на отрицательное значение -1 (должно стать 1)", () -> {
            anotherMark.plus(-1);
    ...
    public void minusWithValue_ShouldHandleValidAndInvalidCases() {
        ...
        executeStep("Уменьшение оценки 4 на 2 = 2", () -> {
            mark.minus(2);
            assertEquals(2, mark.get(), DELTA);
        });

        executeStep("Уменьшение оценки 2 на 10 (должно стать 0)", () -> {
            mark.minus(10);
            assertEquals(0, mark.get(), DELTA);
        });

        Mark anotherMark = new Mark(3);
        executeStep("Уменьшение оценки на отрицательное значение -2 (должно стать 5)", () -> {
            anotherMark.minus(-2);
    ...
    public void set_ShouldAdjustToValidRange() {
        Mark mark = new Mark();
        executeStep("Установка значения 4", () -> {
            mark.set(4);
            assertEquals(4, mark.get(), DELTA);
        });

        executeStep("Установка значения 10 (должно стать 5)", () -> {
            mark.set(10);
            assertEquals(5, mark.get(), DELTA);
        });

        executeStep("Установка отрицательного значения -5 (должно стать 0)", () -> {
            mark.set(-5);
            assertEquals(0, mark.get(), DELTA);
        });
    }
    ...
    private void executeStep(String stepDescription, Runnable action) {
        action.run();
    }
}